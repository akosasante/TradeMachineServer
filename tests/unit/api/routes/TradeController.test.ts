...
describe("TradeController", () => {
    ...
    const recipient = new TradeParticipant({tradeParticipantId: 2, participantType: TradeParticipantType.RECIPIENT,
        team: creatorTeam});
    const tradedMajorPlayer = TradeFactory.getTradedMajorPlayer();
    const tradedMinorPlayer = TradeFactory.getTradedMinorPlayer(undefined, undefined, undefined, {tradeItemId: 2});
    const tradedPick = TradeFactory.getTradedPick();
    ...
    describe("updateTrade method", () => {
        ...
        it("should call the updateItems method", async () => {
            mockTradeDAO.getTradeById.mockReturnValue(testTrade);
            const newMinorPlayer = TradeFactory.getTradedMinorPlayer(
                undefined, creatorTeam, recipientTeam, {tradeItemId: 4});
            ...
            await tradeController.updateTrade(testTrade.id!, updatedTrade.parse());
