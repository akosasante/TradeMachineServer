CREATE OR REPLACE VIEW hydrated_majors (id, name, league, "mlbTeam", "ownerTeam", "eligiblePositions", "mainPosition") as
SELECT player.id,
       player.name,
       player.league,
...
CREATE OR REPLACE VIEW hydrated_minors (id, name, league, "ownerTeam", "minorTeam", position, "minorLeagueLevel") as
SELECT player.id,
       player.name,
       player.league,
       (SELECT json_build_object('id', t.id, 'name', t.name) AS json_build_object
...
CREATE OR REPLACE VIEW hydrated_picks (id, season, type, round, "pickNumber", "currentPickHolder", "originalPickOwner") as
