...
const expression = `
    WITH trade_creator AS (
        ...
        WHERE p."participantType" = '1'::${process.env.ORM_CONFIG}."trade_participant_participanttype_enum"
    ),
         trade_recipients AS (
             SELECT p."tradeId", (SELECT "name" FROM ${process.env.ORM_CONFIG}."team" tm WHERE tm.id = p."teamId")
             FROM ${process.env.ORM_CONFIG}.trade_participant p
             WHERE p."participantType" = '2'::${process.env.ORM_CONFIG}."trade_participant_participanttype_enum"
         ),
         accepted_users AS (
