...
const expression = `
    WITH trade_creator AS (
        ...
        WHERE p."participantType" = '1'::${process.env.PG_SCHEMA}."trade_participant_participanttype_enum"
         ...
         trade_recipients AS (
             SELECT p."tradeId", (SELECT "name" FROM ${process.env.PG_SCHEMA}."team" tm WHERE tm.id = p."teamId")
             FROM ${process.env.PG_SCHEMA}.trade_participant p
             WHERE p."participantType" = '2'::${process.env.PG_SCHEMA}."trade_participant_participanttype_enum"
         ),
         accepted_users AS (
             SELECT t.id, json_array_elements_text(t."acceptedBy"::json) AS "acceptedById"
             FROM ${process.env.PG_SCHEMA}."trade" t
         ),
         players_with_participants AS (
             SELECT i."tradeId",
                    (SELECT "name" FROM ${process.env.PG_SCHEMA}."team" tm WHERE tm.id = i."senderId")    AS "sender",
                    (SELECT "name" FROM ${process.env.PG_SCHEMA}."team" tm WHERE tm.id = i."recipientId") AS "recipient",
                    p.*
             FROM ${process.env.PG_SCHEMA}."trade_item" i
                      RIGHT JOIN ${process.env.PG_SCHEMA}."hydrated_majors" p ON p."id" = i."tradeItemId"
         ...
         prospects_with_participants AS (
             SELECT i."tradeId",
                    ...
                    p.*
             FROM ${process.env.PG_SCHEMA}."trade_item" i
                      RIGHT JOIN ${process.env.PG_SCHEMA}."hydrated_minors" p ON p."id" = i."tradeItemId"
             WHERE i."tradeItemType" = '1'
         ),
         traded_minors AS (
         ...
         picks_with_participants AS (
             SELECT i."tradeId",
                    ...
                    d.*
             FROM ${process.env.PG_SCHEMA}."trade_item" i
                      RIGHT JOIN ${process.env.PG_SCHEMA}."hydrated_picks" d ON d."id" = i."tradeItemId"
    ...
    SELECT t.id                                                                                         AS "tradeId",
           ...
           (SELECT "displayName"
            FROM ${process.env.PG_SCHEMA}."user" u
