...
export class updateHdyrateTradeViewQuery1621487260367 implements MigrationInterface {
    name = 'updateHdyrateTradeViewQuery1621487260367'
    ...
    WITH trade_creator AS (
        SELECT p."tradeId", (SELECT "name" FROM "team" tm WHERE tm.id = p."teamId") FROM trade_participant p WHERE p."participantType" = '1'
         ...
         traded_minors AS (
             SELECT i."tradeId", array_to_json(array_agg(row_to_json(p)))::jsonb AS "tradedMinors" FROM "trade_item" i
