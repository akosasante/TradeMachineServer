...
async function main(prisma: ExtendedPrismaClient) {
    ...
    //
    await prisma.user.findUniqueOrThrow({ where: { email: "test" } });
    // const allUsers = await prisma.user.findMany({ orderBy: { id: "asc" } });
    // console.dir(allUsers);
    // const [firstUser] = allUsers;
    // if (firstUser) {
    //     console.log(`User ${firstUser.passwordResetToken}`);
    // }
    // const res = await prisma.user.update({
    //     where: { id: firstUser.id },
    //     data: { passwordResetToken: null } as unknown as Prisma.UserUpdateInput,
    // });
    //
    // console.dir(res);
    //
    // const updatedUser = await prisma.user.findUnique({ where: { id: firstUser.id } });
